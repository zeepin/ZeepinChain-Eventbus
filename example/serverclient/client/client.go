...
import (
	...
	"github.com/zeepin/ZeepinChain-Eventbus/example/serverclient/message"
)
...
func (client *Client) SyncCall(serverPID *actor.PID) (interface{}, error) {
	future := serverPID.RequestFuture(&message.Request{Who: "ZEEPIN"}, 10*time.Second)
...
func (client *Client) AsyncCall(serverPID *actor.PID) *actor.PID {
	...
	serverPID.Request(&message.Request{Who: "ZEEPIN"}, clientPID)
	return clientPID
}
